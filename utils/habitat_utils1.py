...
def simulate_sensor_data(num_astronauts, simulation_time):
    ...
    # Simulate oxygen levels with controlled noise
    oxygen_level = np.clip(20 + np.sin(time) * 0.5 + np.random.normal(0, 0.05, len(time)), 19.5, 20.5)
    
    # Simulate temperature with realistic fluctuations
    temperature = np.clip(22 + np.cos(time) * 3 + np.random.normal(0, 0.3, len(time)), 19, 25)
    
    # Simulate water usage with normal distribution
    water_usage = np.clip(np.random.normal(3, 1, len(time)), 1, 5)  # Mean 3L, std 1L
    
    # Simulate CO2 levels with minor deviations
    co2_level = np.clip(0.04 + np.sin(time) * 0.02 + np.random.normal(0, 0.002, len(time)), 0.03, 0.05)
    
    # Simulate humidity within a controlled range
    humidity = np.clip(50 + np.cos(time) * 10 + np.random.normal(0, 1, len(time)), 40, 60)
    
    # Simulate power usage with more stability
    power_usage = num_astronauts * 100 + np.random.normal(0, 5, len(time))  # Small deviations
